...
namespace SistemaVendas.Models
{
    public class VendaModel
    {
        ...
        public void Inserir()
        {
            ...
            sql = $"select id from venda where data='{dataVenda}' and Vendedor_id='{Vendedor_Id}' " +
                $"and Cliente_id='{Cliente_Id}' order by id desc limit 1";
            DataTable dt = objDal.RetDataTable(sql);
            string id_venda = dt.Rows[0]["id"].ToString();

            // Serializar o JSON da lista de produtos selecionados e gravá-los na tabela itens_venda
